...
def query(query=None):
    ...
    try:
        if "," in query:
            ...
            data_element_results = DataElement.query \
                                    .filter(and_(query_items[0]>= DataElement.min_group,
                                                 ...
                                                 query_items[1]<= DataElement.max_element))
        else:
            if "x" in query:
                query = int(query, 16)
            else:
        ...
        data_element_results = DataElement.query.filter(
                                query.lower()==func.lower(DataElement.name))
    ...
    if page is not None:
        ...
        page_min = int(page) * de_per_page
        page_max = page_min + de_per_page
        response["pages"] = [page + 1 for page in xrange(last_page)]
        response["data_elements"] = data_elements[page_min:page_max]
